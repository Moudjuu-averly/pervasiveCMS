...
class TenderManager(models.Manager):
    ...
    def search(self, query=None):
        ...
        if query is not None:
            or_lookup = (Q(tender_tittle__icontains=query) |
    ...
    def get_queryset(self):
        return TenderQuerySet(self.model, using=self._db)  # Important!

    def save(self, *args, **kwargs):
        self.slug = slugify(self.tender_tittle)
...
class AnnounceManager(models.Manager):
    """docstring for JobManager."""

    def search(self, query=None):
        qs = self.get_queryset()
        if query is not None:
            or_lookup = (Q(announce_tittle__icontains=query) |
                         Q(category__icontains=query)|
                         Q(content__icontains=query)
                        )
            qs = qs.filter(or_lookup).distinct() # distinct() is often necessary with Q lookups
        return qs
    def get_queryset(self):
        return AnnounceQuerySet(self.model, using=self._db)  # Important!

    def save(self, *args, **kwargs):
        self.slug = slugify(self.announce_tittle)
        super(Tender, self).save(*args, **kwargs)
